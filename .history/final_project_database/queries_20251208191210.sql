...
final String getNumReadTot = "SELECT COUNT(DISTINCT u.userId) AS numReadTot "
                                        ...
                                        + "JOIN history AS h ON u.userId = h.userId "
                                        + "WHERE h.has_read = 1;";
-- Purpose: Retrieve user information based on username (for login or validation)
-- Context: UserService.java
-- URL: http://localhost:8080/login
final String sql = "select * from user where username = ?";


-- Purpose: Register a new user by inserting their username, password, first name, and last name
-- Context: UserService.java
-- URL: http://localhost:8080/register
final String registerSql = "insert into user (username, password, firstName, lastName) values (?, ?, ?, ?)";

-- Purpose: Retrieve all books that the logged-in user has wishlisted
-- Context: WishlistService.java
...
-- Purpose: Add a book to the user's wishlist (set has_wishlisted = TRUE)
-- Context: WishlistService.java
-- URL: http://localhost:8080/wishlist/{bookId}/wishlisted/true
...
-- Purpose: Remove a book from the user's wishlist (set has_wishlisted = FALSE)
-- Context: WishlistService.java
-- URL: http://localhost:8080/wishlist/{bookId}/wishlisted/false
final String removeWishlistSql = "UPDATE history SET has_wishlisted = FALSE WHERE userId = ? AND bookId = ?";


-- Purpose: Check whether a specific book is already on the user's wishlist
-- Context: WishlistService.java
-- URL: http://localhost:8080/books/{bookId}
